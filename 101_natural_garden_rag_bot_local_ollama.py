...
class RAGBot:
    def __init__(self, csv_path: str, embed_model: str, gen_model: str, top_k: int = 4):
        ...
        # 如果你的 Python 跑在同一台 Jetson：不用設 base_url
        # 如果是跨機器：ChatOllama(base_url="http://<JETSON_IP>:11434", ...)
        self.llm = ChatOllama(
            model="llama3.2:1b",    # 本地模型名稱
            temperature=0.2,
            streaming=False,        # 你要串流也可以設 True，自行加上 callback
            # base_url="http://localhost:11434",  # 預設就是這個
        )
